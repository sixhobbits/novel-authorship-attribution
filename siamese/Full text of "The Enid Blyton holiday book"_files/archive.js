...
(function ($) {
  ...
  AJS.tvAD = function (elm) {
    ...
    if (in_show.length) {
      ...
      var chan = in_show.attr('data-chan');
      var show = $(elm).text();
    } else {
      var chan = $(elm).text();
      var id = 'menu-chan-' + chan;
      var div = $('#' + id);
      if (div.length) {
        div.toggle();
        return false;
    ...
    if (in_show.length) url += '&program=' + encodeURIComponent(show);
    log(url);

    $.get(url, function (htm) {
      if (in_show.length) $(elm).parent().append('<div data-chan="' + chan + '" class="shows"><h5>Air date</h5><div>' + htm + '</div></div>');else $(elm).parent().append('<div id="' + id + '" data-chan="' + chan + '" class="shows"><h4>Show</h4><div>' + htm + '</div></div>');
    });

    return false;
  };

  AJS._check_results_for_dupe = function () {
    var n = 0;
    var map = {};
    $('.ikind.in  .results:visible  .item-ia').each(function (k, v) {
      n++;
      var id = $(v).attr('data-id');
      if (typeof map[id] == 'undefined') map[id] = 1;else {
        map[id]++;
        log('DUPE ' + id);
      }
    });
  ...
  AJS.nav_tophat_setup = function () {
    ...
    if (AJS.isTouchDevice()) $('body').addClass('touch');else AJS.about();
    ...
    if ($('body').hasClass('nav-tophat-over')) {
      ...
      $('#nav-tophat').on('show.bs.collapse', function () {
        ...
        var max = Math.max.apply(Math, $('#nav-tophat .fivecolumns .posrel').map(function (k, v) {
          return $(v).height();
        }));
        if (max > navHT) {
    ...
    $('#nav-tophat').on('show.bs.collapse', function () {
      $('#navwrap1').addClass('hatted');
    }).on('hide.bs.collapse', function () {
      $('#navwrap1').removeClass('hatted');
    });

    $('.navbar a.navia-link').on('click', function (evt) {
      // are we in "mobile nav / xs width" mode?
      var xs = $('#nav-tophat-helper:visible').length ? false : true;
      ...
      $('#nav-tophat').on('hidden.bs.collapse', function () {
        $('.navbar .dropdown-ia').removeClass('hatted');
      });
      ...
      $(document).one('click.tophat.nixer', function (evt) {
        ...
        var $hat = $('#nav-tophat');
        // THIS DOESNT QUITE WORK ACTUALLY...
        ...
        */
        ...
        if ($hat.hasClass('in')) $hat.collapse('hide');
      });
  ...
  AJS.isTouchDevice = function () {
    return "ontouchstart" in window || window.DocumentTouch && document instanceof DocumentTouch;
  };

  AJS.footer = function () {
    if (!AJS.isTouchDevice() && typeof $ != 'undefined' && typeof $.fn.tooltip != 'undefined') {
      $('.navbar [data-toggle="tooltip"], .container-ia [data-toggle="tooltip"], #nav-tray [data-toggle="tooltip"], #cols [data-toggle="tooltip"], #ccvis-banner [data-toggle="tooltip"]').tooltip({});
      //$('body').addClass('glyphs-as-text');
    }
  };

  AJS.wrapdark_watcher = function () {
  ...
  AJS.flash_click = function (flashy) {
  ...
  AJS.emulate_setup = function (game) {
    ...
    $(window).on('resize  orientationchange', function (evt) {
      $.doTimeout('AJS.theatre_controls_position' + evt.type, 250, AJS.theatre_controls_position);
    });
  };
  ...
  AJS.emulate = function () {
    ...
    if (EM && (canvas.webkitRequestFullScreen || canvas.mozRequestFullScreen || canvas.requestFullScreen)) {
      ...
      /**/if ('onfullscreenchange' in document) {
        document.addEventListener('fullscreenchange', EM.fullScreenChangeHandler);
      } else if ('onmozfullscreenchange' in document) {
        document.addEventListener('mozfullscreenchange', EM.fullScreenChangeHandler);
      } else if ('onwebkitfullscreenchange' in document) {
        document.addEventListener('webkitfullscreenchange', EM.fullScreenChangeHandler);
      }
    }
  ...
  AJS.theatre_controls_position = function ($selector, pegTop, width, height) {
    // We have lots of callers!  video, software, texts.
    // So sort out our args and where we gonna "peg"/glue things to...
    var video = height && !$selector;
    if (!video) {
      if (!$selector) $selector = $('#canvas'); //software emulation
      if (!$selector.length) return; // protect against emulated embeds and undef...
      height = $selector.height();
      width = $selector.width();
      if (typeof pegTop != 'undefined') $('#theatre-controls').offset({ top: pegTop });
    }
    ...
    $('#theatre-controls').css({ height: height, visibility: 'visible', right: Math.max(20, right_gutter_width / 2) });
  };
  ...
  AJS.booksize = function () {
    ...
    if ($("#texty").hasClass("lendable-book")) {
      ...
      var lendableBooksize = function lendableBooksize(evt) {
      ...
      lendableBooksize();
      // This is a hack to change the bg on archive.org details page
    ...
    var vidW, vidH;
    for (var vidW = maxW; vidW >= 180; vidW--) {
    ...
    setTimeout(function () {
      if (AJS.booksize_controls) AJS.booksize_controls();
    }, 500);
  ...
  AJS.booksize_controls = function () {
  ...
  AJS.popcornsize = function () {
    ...
    $(window).on('resize  orientationchange', function (evt) {
      $.doTimeout('AJS.popcorn_sizer' + evt.type, 250, function () {
        _resizer();
      });
    });
  };
    ...
    render: function render() {
      ...
      var options = this.state.options.map(function (option) {
        return React.createElement(
          ...
          React.createElement(
            'div',
            { className: 'facell' },
            React.createElement('input', { onClick: _this.handleClick, type: 'checkbox',
              name: option.val,
              value: option.val, ref: option.val })
          ),
          React.createElement(
            'div',
            { className: 'facell' },
      ...
      if (loading) {
        loadnote = React.createElement(
          ...
          'loading filters... ',
          React.createElement('img', { style: { 'width': '25px' }, key: 'loading', src: '/images/loading.gif'
          })
        );
      } else {
        for (page = 1; page <= npages; page++) {
          if (this.state.page === page) paging.push(React.createElement(
            'div',
            { key: 'p' + page, className: 'topinblock' },
            page
          ));else paging.push(React.createElement(
            'a',
            { href: '#', key: 'p' + page, onClick: this.pageClick },
            page
          ));
          paging.push(' ');
        }
        if (this.state.page < npages) {
          paging.push(React.createElement(
            'a',
            { href: '#', key: 'pp' + page, onClick: this.pageClick },
  ...
  AJS.morf = function (lnk, switchToA2Z) {
    // MORe Facets
    var href = $(lnk).attr('href');

    if (switchToA2Z && switchToA2Z[0] === '-') $.cookie(switchToA2Z.substr(1) + 'A2Z', 1, { path: '/', expires: 1 });else if (switchToA2Z) $.cookie(switchToA2Z + 'A2Z', null, { path: '/', expires: 1 });
    ...
    var contents = React.createElement(
      ...
      React.createElement(
        ...
        React.createElement('div', { className: 'modal-body', id: props.selectorID + '-body' })
      )
    );
  ...
  AJS.popover_menu = function (selector, opts) {
    // setup options
    var options = {
  ...
  AJS.date_switcher = function (htm) {
    ...
    $(selector).on('shown.bs.tooltip', function () {
      ...
      $(selector).parent().find('.date_switcher').on("click", function () {
  ...
  AJS.lister = function () {
    ...
    */
    ...
    var availWidth = C234 - C3;
    availWidth -= 20; // yah, this was empirically observed / made up -- get some science, girl!
  ...
  AJS.lists_v_tiles_setup = function (cookie_range) {
    // does user have any cookied preferences?
    var defaults = cookie_range == 'search' ? 'lists' : 'tiles';
    ...
    if (prefer == 'lists' || !prefer && defaults == 'lists') {
      ...
      $('body').removeClass('lists' + (checked ? '' : ' showdetails')).addClass('tiles');
    }
  };

  AJS.showdetails_toggle = function (btn, cookie_range) {
    var $body = $('body');
  ...
  AJS.selector = function () {
    var inTab = $('.welcome .tabby.in .tabby-text').text().toLowerCase();
    ...
    if (inTab) {
      ...
      if (inTab == 'collection' || inTab == 'collections' || inTab == 'uploads') {
        selector = '#' + $(tmp + ' div.ikind.in:first').attr('id');
      } else {
        var ikind = $(tmp + ' .ikind.stealth.in:first').text().toLowerCase().replace(/ /, '-');
      ...
      if (!$(selector).length) selector = '#ikind-search'; //xxxxxxxxx  SHORE THIS UP, GIRL!
    }
  ...
  AJS.parent_hover = function ($selector) {
    $selector.find('.item-ia:not(.hov):not(.collection-ia):not(.account-ia)').addClass('hov').mouseover(function (e) {
      $(e.currentTarget).find('.item-parent').addClass('hoverin');
    }).mouseout(function (e) {
      $(e.currentTarget).find('.item-parent').removeClass('hoverin');
    });
  };

  AJS.tiler = function (selector, noRecall) {
    var tileMarginH = 30;
    var tileMarginW = 17;
    ...
    if (!selector) selector = AJS.selector();
    ...
    if ($(window).width() != AJS.tilerPREV.winW) {
      ...
      var tileWidth = $('.item-ia:first').outerWidth() >= TILE_WIDTH ? TILE_WIDTH : TILE_WIDTH_SMALL;
      ...
      log(dbug + 'With gutter margin: ' + gutter + ', had ' + extra + ' extra pixels');
    ...
    if (!AJS.tilerPREV.unsourced[selectorID]) {
      ...
      $selector.find('.item-ia img[source]').each(function (k, v) {
        v = $(v);
        v.attr({ src: v.attr('source'), 'onError': '$(this).attr("src","/images/notfound.png")' }).removeAttr('source');
      });
      AJS.tilerPREV.unsourced[selectorID] = 1;
    }

    // we track where previous tile was placed
    AJS.colPREV = -1;
    $selector.find('.item-ia').not('.mobile-header').css({ visibility: 'hidden' }).each(function (idx, val) {
      ...
      var col = 0;
      // can uncomment for easier testing of "tactic"s
      //$val.find('.ttl, .collection-title a').text(' TITLE #'+idx);
      var tactic = 3;
      // NOTE: regardless of tactic, always use 1st column STILL with a top == 0 first!
      if (tactic && AJS.tilerPREV.nPerRow > 1 && idx >= AJS.tilerPREV.nPerRow) {
        if (tactic == 3) {
          ...
          for (var i = 0; i < AJS.tilerPREV.nPerRow; i++) {
            if (i != colTEST && cmpTEST > tops[i]) {
              // log("BETTER COL FOR TILE #"+idx+": " + i);
              col = i;
              break;
            }
          }
        } else if (tactic == 2) {
          for (var i = AJS.tilerPREV.nPerRow - 1; i >= 0; i--) {
            // tactic #2: "test fit" next tile in every column.
            // pick the column where the tile TOP would be higher up than all other
            // candidate tile BOTTOM y values
            var t1 = tops[i];
            for (var e = 0; e < AJS.tilerPREV.nPerRow; e++) {
              if (e == i) continue;
              if (t1 - $valHT > tops[e]) break; // nonideal
            }
            if (e == AJS.tilerPREV.nPerRow) {
              log("BEST COL FOR #" + idx + " SO FAR: " + i);
              col = i;
            }
          }
        } else if (tactic == 1) {
          // tactic #1: "use column with highest up tops value, but dont repeat/pick same prior column"
          var mintop = Number.MAX_VALUE;
          for (var i = 0; i < AJS.tilerPREV.nPerRow; i++) {
            if (AJS.tilerPREV.nPerRow > 2 && i == AJS.colPREV) continue;
            if (tops[i] < mintop) {
              mintop = tops[i];
              col = i;
            }
    ...
    if (nImgZeroHeight && !noRecall) {
      log(nImgZeroHeight + ' img (still w/o height); recall tiler(' + selector + ')...' + nImgZeroHeightStr);
  ...
  AJS.ikind = function (lnk, id) {
    log('ikind(' + id + ')');
    $('#' + id).parent().find('div.ikind.in').hide();
    ...
    var $sel = $('#' + id).parents('.tabby-data').find('select.ikind-mobile');
  ...
  AJS.ikind_mobile_change = function (elm) {
    ...
    var $ikind = $ikinds.filter(function (index) {
      return $(this).text() === $selopt.text();
    });
  ...
  AJS.popState = function (pageType) {
    ...
    /* not working when flip around tabs after and then do [back] ...
    var hash2arg=false;
    ...
    */
    ...
    if (AJS.pushState) {
      $(window).on('popstate', function (e) {
        //run on page load or browser "back"
        tabPick();
      });
    }
  ...
  AJS.tabby = function (lnk, id) {
    ...
    if (AJS.tabby_no_pushState_next_click) {
      ...
      if (AJS.pushState && typeof history.pushState != 'undefined') {
        var href = $(lnk).attr('href');
        var locNOW = location.href;
        var locNEW = location.protocol + '//' + location.host + href;
        if (locNOW != locNEW) {
    ...
    if (AJS.pushState) return false;else return true;
  };

  AJS.colclick = function (e) {
    var $e = $(e);
  ...
  AJS.list_remove_item = function (xer) {
    if ($('body').hasClass('lists')) {
      $('.tiles-button:visible').click(); //sorry! this only works in tiles mode now...
    }
  ...
  AJS.manage = function () {
    ...
    var ids = $(".item-ia[data-id]").toArray().reduce(function (a, b) {
      ...
      return "" + a + (id == "__mobile_header__" ? "" : id + ",");
    }, "");
    log('MANAGE: ' + ids);
    ...
    if (ids !== "") {
      ...
      var $form = $('<form id="manage-ids" method="POST" action="/manage/" style="visibility:hidden;position:absolute;top:0;left:0;width:1px;height:1px;"><input type="text" name="identifier" value="' + ids + '"/></form>');
  ...
  AJS.drawPDF = function (identifier, pageinfo) {
    ...
    var urlstart = '/download/' + identifier + '/' + identifier + '_pdf.zip/' + identifier + '_pdf/' + identifier + '_';
    ...
    for (var key in pageinfo) {
      ...
      var year = key.substr(0, 4);
      var month = parseInt(key.substr(4, 2), 10);
      ...
      if (yearmonth != lastyearmonth) {
        /**/if (month == 1) monthName = "January";else if (month == 2) monthName = "February";else if (month == 3) monthName = "March";else if (month == 4) monthName = "April";else if (month == 5) monthName = "May";else if (month == 6) monthName = "June";else if (month == 7) monthName = "July";else if (month == 8) monthName = "August";else if (month == 9) monthName = "September";else if (month == 10) monthName = "October";else if (month == 11) monthName = "November";else if (month == 12) monthName = "December";else if (month === undefined) monthName = "Single Page PDFs";else /*          */monthName = "Unknown_" + month;

        if (multi_year) monthName = year + ' ' + monthName;

        // make header/a that shows/hides a hidden div after it with the
        // month's data
        str += (str == '' ? '' : '</div><!--mo--><br/>') + '<a href="#' + monthName + ',' + year + '" onclick="$(\'#m' + yearmonth + '\').toggle(); return false;"><span class="iconochive-folder"></span> ' + monthName + '</a>' + '<div class="mo" id="m' + yearmonth + '">';
      }
      ...
      if (skip_day) {
        ...
        str += '<div class="day"><a href="#' + yearmonth + '-' + day + '" onclick="$(\'#m' + yearmonth + 'd' + day + '\').toggle();return false;"><span class="iconochive-folder"></span> ' + day + '</a> <div class="pages" id="m' + yearmonth + 'd' + day + '">';
      }
  ...
  AJS.grafs = function () {
    ...
    $.get('/details/' + identifier + '&grafs=1&v=2', function (htm) {
      var tok = "<h2>";
      var a = htm.split(tok);
      if (a.length >= 3) {
  ...
  AJS.quick_down = function (id, target) {
    if (!$('.format-group.in').length) {
      // no set of files for a single format showing... yet!
      var format = $(target).text();
      var $formatGroup = $('#' + id).parents('.format-group');

      $('.download-button').html(format + ' FILES');
  ...
  // parse a CGI arg
  AJS.arg = function (theArgName, try_full) {
  ...
  AJS.search_this_list = function (url, form) {
    if (url === '') {
    ...
    url += query === '' ? '' : (location.search === '' ? '?' : '&') + 'and[]=' + query;
  ...
  AJS.about = function (lnk) {

    AJS.aboutHide = function () {
      $('#internet-archive').fadeOut('slow', function () {
        $('#internet-archive').remove();
        $('#exit-er').show();
      });
      $('#nav-abouts').slideToggle(function () {
        $('#nav-abouts').remove();
      });
    };
    ...
    $('.navbar-nav:first').on('mouseenter focusin', function (evt) {
      ...
      var $e = $(evt.target);
      if (!($e.is('ul') || $e.hasClass('navbar-brand') || $e.hasClass('iconochive-logo') || $e.hasClass('about-hider'))) return;

      // if nav search is open -- no About opening!
      if ($('#nav-search-in:visible').length) return;
  ...
  AJS.scrolled = function () {
    ...
    if (newtop > check) {
      ...
      if (!AJS.more_searching) $(selector + ' > a').click();
    }
  };

  AJS.page_map = {};
  AJS.more_search = function (lnk, url, page_now) {
    ...
    if (typeof AJS.page_map[pageKey] == 'undefined') AJS.page_map[pageKey] = page_now ? page_now : 1;
    ...
    url += page;
    var urlreplace = location.protocol + '//' + location.host + url;
    url += '&scroll=1';
    ...
    $.get(url, function (htm) {
      ...
      var $selector = $(selector + ' .results');
      htm = AJS.addNotes(AJS.addingNotesKind, htm);
      $selector.append(htm);
  ...
  AJS.embed_codes_adjust = function () {
    ...
    for (var embid in embids) {
      ...
      $('body').prepend($('<div/>').attr({ 'id': embid + 'Shadow', 'class': 'textarea-invert-readonly roundbox5' }).css({ 'position': 'absolute',
        'visibility': 'hidden',
        'top': 60,
        'left': 10,
  ...
  AJS._modal_add = function (selector, conf) {
    log(conf);
...
');
  ...
  AJS.modal_beta = function (lnk, conf) {
    //xxxn
...
\n\
    ...
    $('#modal_beta_form').on('submit', function () {
      ...
      // we're polite
      $(selector + ' h5').fadeIn('slow');
      $(selector + ' form').fadeOut('slow');

      setTimeout(function () {
        // AJAX form post, keeping page where it is
        $.post('/services/exit.php', conf.postdata, function (htm) {
    ...
    $(selector).modal('show').on('hidden.bs.modal', function (evt) {
  ...
  AJS.modal_go = function (lnk, conf) {
    ...
    if (conf.login && $.cookie('logged-in-user') === null) {
      ...
      $.get(url, function (htm) {
        ...
        $(selector + ' .modal-title').text('Nearly there!  Please login first');

        $(selector).modal('show');
        // this allows us to effectively rip off the header/nav and footer
        // if we are accessing a full page (eg: account login page)
        if ($(htm).find('.container-ia > div').length) htm = $(htm).find('.container-ia > div').get(0);

        $(selector + '-body').html(htm);

        var $form = $(selector + '-body form:has(input[type=submit])');
    ...
    if (conf.auto_remove) {
      ...
      $(selector).on('hidden.bs.modal', function () {
        $(selector).remove();$('body').removeClass('blurry');
      });
    }
    ...
    if (conf.center) $(selector + ' .modal-dialog').center(); // vertically center
  ...
  AJS.check1 = function (obj, selector) {
    var val = $(obj).attr('name');
    if (obj.checked) {
      // toggle others to unchecked
      if (selector) selector = $(selector);else selector = $(obj).parents('form');
  ...
  AJS.suffixFmt = function (val, axis) {
  ...
  AJS.suffixFmtPercent = function (val, axis) {
    return AJS.suffixFmt(val, axis) + '%';
  };

  AJS.descript = function () {
  ...
  AJS.plot = function (id, cfg, fmt_fn, pts, many) // for archive.org/services/views.php    and   TV graphs
  {
    ...
    cfg.series = {
      bars: {
        show: true,
        barWidth: many && cfg.barWidth ? cfg.barWidth : (cfg.barWidth ? cfg.barWidth : cfg.dayBarsNoPoints ? 1 : 7) * 86400 * 1000, //1day|1week
        fill: 0.6,
        color: "#385C74"
      },
      color: "#385C74",
      points: { show: cfg.dayBarsNoPoints ? false : true }
    };
    ...
    $('#' + id).bind("plothover", function (event, pos, item) {
      ...
      if (window.GraphPriorIndex[id] != item.dataIndex) {
        ...
        pegTo = pegTo + ':' + (pegTo == 'right' ? $graf.width() - (pos.pageX - off.left - 20) : pos.pageX - off.left + 20) + 'px';
        //log(pegTo);

        $('#' + cfg.tip.id).append('<div id="gtip" class="roundbox2" style="position:absolute; top:' + (pos.pageY - off.top) + 'px; ' + pegTo + '; padding:2px 5px; border:1px solid gray; background-color:#D8DEDE;  color:#385C74; opacity:.9; ">' + str + '</div>');
      }
    });
  };
  ...
  AJS.welcome_recolor = function (e) {
    ...
    var avgVal = Math.round($(dominantColorRGB).map(function (idx, val) {
      return val;
   ...
   */
  AJS.makeMouseElementAccessible = function (element) {
    $(element).attr("tabindex", "0").on("keypress keydown", function (e) {
      if (e.type != "click") {
  ...
  AJS.autocomplete = function (selector, options, multiValSplitChar) {
  ...
  AJS.addNotes = function (kind, htm) {
    if (false && !$('body').hasClass('devbox')) return htm; //xxxc
...
'));
  ...
  AJS.editedNote = function (e) {
    ...
    if (action == 'Save' || action == 'Remove') {
      ...
      var url = '/bookmarks.php?identifier=' + identifier + '&kind=' + AJS.addingNotesKind + '&add_comment=' + encodeURIComponent(val);
      log('get ' + url);
      $.get(url, function (htmReply) {
        var htm = val ? 'Note: "<span>' + val.replace(/</g, '&lt;').replace(/>/g, '&gt;') + '</span>" <span class="edit">(<a href="#" onclick="return AJS.editNote(this)">edit</a>)</span>' : AJS.addNoteHTM.html();
        $origNote.html(htm).show(); // show the "Note: ..." section again (with now appropriately updated innards)
  ...
  AJS.thumbzilla = function (id) {
    ...
    $(AJS.thumbzillas).each(function (key, val) {
      ...
      if (TV) {
        var $end = start + TV3.CLIP_SEC_MAX2;
    ...
    $('#opscreen1M').hide().css({ 'visibility': 'visible' }).fadeIn('slow').one('click.opscreen1M.nixer', function (evt) {
  ...
  AJS.nav_tophat_wb_setup = function () {
    // Configuration
    ...
    function search_or_calendar(query) {
      if (query.indexOf('http://') === 0 || query.indexOf('https://') === 0 || query.match(/[\w\.]{2,256}\.[a-z]{2,4}/gi)) {
            ...
            source: function source(query, process) {
              ...
              return $.get(HOSTS_ENDPOINT + encodeURIComponent(query), function (data) {
                if (typeof data.hosts !== 'undefined' && data.hosts.length > 0) {
                  ...
                  return process([{ "display_name": query }]);
                } else {
                  return $.get(ANCHORS_ENDPOINT + encodeURIComponent(query), function (data) {
                    if (typeof data !== 'undefined' && data.length > 0) {
                      return process(data.slice(0, 5));
                    }
                  });
                }
              });
   ...
   */
  AJS.PopupLink = function PopupLink() {
   ...
   * appropriate ID.
   */
  AJS.setUpCreativeCommonsLicenseLink = function setUpCreativeCommonsLicenseLink() {
...
(function ($) {
  var a = {},
      c = "doTimeout",
      d = Array.prototype.slice;$[c] = function () {
    return b.apply(window, [0].concat(d.call(arguments)));
  };$.fn[c] = function () {
    var f = d.call(arguments),
        e = b.apply(this, [c + f[0]].concat(f));return typeof f[0] === "number" || typeof f[1] === "number" ? this : e;
  };function b(l) {
    var m = this,
        h,
        k = {},
        ...
        f = n[1],
        j = n[2],
        p = n[3];if (typeof f !== "string") {
      i--;f = l = 0;j = n[1];p = n[2];
    }if (l) {
      h = m.eq(0);h.data(l, k = h.data(l) || {});
    } else {
      if (f) {
        k = a[f] || (a[f] = {});
      }
    }k.id && clearTimeout(k.id);delete k.id;function e() {
      if (l) {
        h.removeData(l);
      } else {
        if (f) {
      ...
      if (k.fn) {
        j === undefined ? e() : k.fn(j === false);return true;
      } else {
        e();
      }
    }
...
(function ($) {
  $(function () {
    ...
    if (typeof archive_setup != 'undefined') {
      ...
      $(function () {
        for (var i = 0; i < archive_setup.length; i++) {
          archive_setup[i]();
        }
      });
    }
