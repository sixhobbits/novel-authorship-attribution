...
def __scale_each_test(trainset, testset):
    ...
    for tc in testset:
        ...
        scaled_train = scaled[:-1]
    return (scaled_train,np.array(scaled_test))
...
def __scale_features(dataset, scale_individually=False):
    """
    Check if features should and could be scaled.

    :param dataset: an array of arrays
    :param scale_individually: boolean -- scale each case individually if True
...
def __load_arg_parser():
    arg_parser = argparse.ArgumentParser(description="Perform data manipulation to extract features and finally run "
                                                     "classification.")
    group_get_model = arg_parser.add_mutually_exclusive_group(required=False)
...
if __name__ == "__main__":
    ...
    if args.split:  # if splitting of 1 data set into train and test
        ...
        test = __scale_features(test, scale_individually=True)
        # train, test = __scale_each_test(train, test)
